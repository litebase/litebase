...
func TestRestore(t *testing.T) {
	test.RunWithApp(t, func(app *server.App) {
		t.Run("CopySourceDatabaseToTargetDatabase", func(t *testing.T) {
			...
			// There should be 7 entries in the target directory: 5 range files, metadata file, and range index
			// Note: _RANGE_LOG file is no longer created due to data range simplification
			if len(entries) != 7 {
