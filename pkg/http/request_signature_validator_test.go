...
func TestRequestSignatureValidator(t *testing.T) {
	test.RunWithApp(t, func(app *server.App) {
		...
		t.Run("InvalidRequestToken", func(t *testing.T) {
		...
		t.Run("CaseInsensitiveHeaders", func(t *testing.T) {
			db := test.MockDatabase(app)
			databaseUrl := fmt.Sprintf("localhost:8080/v1/databases/%s/%s", db.DatabaseName, db.BranchName)
		...
		t.Run("ComplexQueryParams", func(t *testing.T) {
			db := test.MockDatabase(app)
			databaseUrl := fmt.Sprintf("localhost:8080/v1/databases/%s/%s", db.DatabaseName, db.BranchName)
		...
		t.Run("WithBodyContent", func(t *testing.T) {
			db := test.MockDatabase(app)
			databaseUrl := fmt.Sprintf("localhost:8080/v1/databases/%s/%s", db.DatabaseName, db.BranchName)
