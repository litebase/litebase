...
func TestTieredFileSystemDriver(t *testing.T) {
	test.RunWithApp(t, func(app *server.App) {

		t.Run("New", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)

			fs2 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/object"),
			)
			...
			defer func() {
				if err := tieredFileSystemDriver.Shutdown(); err != nil {
					t.Fatalf("failed to shutdown tieredFileSystemDriver: %v", err)
				}
			}()
		...
		t.Run("ConcurrentFilesAccess", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local_concurrent"),
			)

			fs2 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/object_concurrent"),
			)
					...
					for i := range numFiles {
						...
						if i%2 == 0 {
							...
							if err != nil {
								...
								if err := f.Close(); err != nil {
									errCh <- fmt.Errorf("goroutine %d: close error: %w", gid, err)
								}
		...
		t.Run("ClearFiles", func(t *testing.T) {
			...
			err := os.WriteFile(app.Config.StorageLocalPath+"/local/test_1", []byte("test"), 0600)
			...
			if _, err := os.Stat(app.Config.StorageLocalPath + "/local/test_1"); !os.IsNotExist(err) {
		...
		t.Run("MarkFileUpdated", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("Mkdir", func(t *testing.T) {
			...
			err := tieredFileSystemDriver.Mkdir("test_Mkdir/", 0750)
			...
			// A new file should exist in local storage
			info, err := tieredFileSystemDriver.Stat("test_Mkdir/")
		...
		t.Run("MkdirAll", func(t *testing.T) {
			...
			err := tieredFileSystemDriver.MkdirAll("test_MkdirAll/test/test/", 0750)
			...
			// A new file should exist in local storage
			info, err := tieredFileSystemDriver.Stat("test_MkdirAll/test/test/")
		...
		t.Run("Open", func(t *testing.T) {
			...
			_, err := tieredFileSystemDriver.Open("test_Open")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.Open should return os.IsNotExist error, got %v", err)
			}

			tieredFile, err := tieredFileSystemDriver.Create("test_Open")
			...
			if err := tieredFile.Close(); err != nil {
				t.Error(err)
			}

			tieredFile, err = tieredFileSystemDriver.Open("test_Open")
		...
		t.Run("OpenDurableFile", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			err = fs2.WriteFile("test.txt", []byte("test"), 0600)
			...
			if _, err := tieredFile.Seek(0, io.SeekStart); err != nil {
				t.Error(err)
			}
		...
		t.Run("OpenFile", func(t *testing.T) {
			...
			_, err := tieredFileSystemDriver.OpenFile("test_OpenFile", os.O_RDONLY, 0600)

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.OpenFile should return os.IsNotExist error, got %v", err)
			}

			tieredFile, err := tieredFileSystemDriver.Create("test_OpenFile")
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test_OpenFile", os.O_RDONLY, 0600)
			...
			// Create a new file
			_, err = tieredFileSystemDriver.Create("test2_OpenFile")
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test2_OpenFile", os.O_WRONLY, 0600)
			...
			// Create a new file
			_, err = tieredFileSystemDriver.Create("test3_OpenFile")
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test3_OpenFile", os.O_RDWR, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test4_OpenFile", os.O_CREATE, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test4_OpenFile", os.O_CREATE|os.O_RDWR, 0600)
		...
		t.Run("ReadDir", func(t *testing.T) {
			...
			defer func() {
				if err := tieredFileSystemDriver.Shutdown(); err != nil {
					t.Errorf("TieredFileSystemDriver.Shutdown should return nil, got %v", err)
				}
			}()
		...
		t.Run("ReadFile", func(t *testing.T) {
			...
			_, err := tieredFileSystemDriver.ReadFile("test_ReadFile")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.ReadFile should return os.IsNotExist error, got %v", err)
			}

			err = fs2.WriteFile("test_ReadFile", []byte("test"), 0600)
			...
			data, err := tieredFileSystemDriver.ReadFile("test_ReadFile")
		...
		t.Run("ReleaseOldestFile", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			if _, err := tieredFileSystemDriver.OpenFile("test2", os.O_RDWR|os.O_CREATE, 0600); err != nil {
				t.Error(err)
			}
		...
		t.Run("ReleaseOldestFile_WhileReading", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			if _, err := tieredFile1.Write([]byte("helloworld")); err != nil {
				t.Error(err)
			}

			data := make([]byte, 5)

			if _, err := tieredFile1.Seek(0, io.SeekStart); err != nil {
				t.Error(err)
			}
			...
			if err := tieredFileSystemDriver.Flush(); err != nil {
				t.Error(err)
			}
			...
			if _, err := tieredFile1.Seek(5, io.SeekStart); err != nil {
				t.Error(err)
			}
		...
		t.Run("Remove", func(t *testing.T) {
			...
			err := tieredFileSystemDriver.Remove("test_REMOVE")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.Remove should return os.IsNotExist error, got %v", err)
			}

			err = fs2.WriteFile("test_REMOVE", []byte("test"), 0600)
			...
			_, err = tieredFileSystemDriver.ReadFile("test_REMOVE")
		...
		t.Run("Rename", func(t *testing.T) {
			...
			err := tieredFileSystemDriver.Rename("test_Rename", "test_Rename_2")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.Rename should return os.IsNotExist error, got %v", err)
			}

			err = fs2.WriteFile("test_Rename.txt", []byte("test"), 0600)

			if err != nil {
				t.Error(err)
			}

			err = tieredFileSystemDriver.Rename("test_Rename.txt", "test_Rename_2.txt")

			if err != nil {
				t.Error(err)
			}

			_, err = tieredFileSystemDriver.ReadFile("test_Rename.txt")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.ReadFile should return os.IsNotExist error, got %v", err)
			}

			data, err := tieredFileSystemDriver.ReadFile("test_Rename_2.txt")
		...
		t.Run("Stat", func(t *testing.T) {
			...
			_, err := tieredFileSystemDriver.Stat("test_Stat")

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.Stat should return os.IsNotExist error, got %v", err)
			}

			err = fs2.WriteFile("test_Stat.txt", []byte("test"), 0600)

			if err != nil {
				t.Error(err)
			}

			info, err := tieredFileSystemDriver.Stat("test_Stat.txt")
		...
		t.Run("Truncate", func(t *testing.T) {
			...
			err := tieredFileSystemDriver.Truncate("test_Truncate", 4)

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.Truncate should return os.IsNotExist error, got %v", err)
			}

			err = fs2.WriteFile("test_Truncate.txt", []byte("test"), 0600)

			if err != nil {
				t.Error(err)
			}

			err = tieredFileSystemDriver.Truncate("test_Truncate.txt", 2)

			if err != nil {
				t.Error(err)
			}

			info, err := tieredFileSystemDriver.Stat("test_Truncate.txt")
		...
		t.Run("TieredFileSystemDriver_WriteFile", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("TieredFileIsReleasedWhenTTLHasPassed", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("TieredFileIsFlushedToDurableStorageAfterUpdate", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("LocalFileWithDifferentAccessFlags", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			// Test open read only file that does not exist
			_, err := tieredFileSystemDriver.OpenFile("test_LocalFileWithDifferentAccessFlags", os.O_RDONLY, 0600)

			if err == nil || !os.IsNotExist(err) {
				t.Errorf("TieredFileSystemDriver.OpenFile should return os.IsNotExist error, got %v", err)
			}

			tieredFile, err := tieredFileSystemDriver.Create("test_LocalFileWithDifferentAccessFlags")
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test_LocalFileWithDifferentAccessFlags", os.O_RDONLY, 0600)
			...
			// Create a new file
			_, err = tieredFileSystemDriver.Create("test2_LocalFileWithDifferentAccessFlags")
			...
			// Test opening a write only file
			tieredFile, err = tieredFileSystemDriver.OpenFile("test2_LocalFileWithDifferentAccessFlags", os.O_WRONLY, 0600)
			...
			// Create a new file
			_, err = tieredFileSystemDriver.Create("test3_LocalFileWithDifferentAccessFlags")
			...
			// Test opening a read write file
			tieredFile, err = tieredFileSystemDriver.OpenFile("test3_LocalFileWithDifferentAccessFlags", os.O_RDWR, 0600)
			...
			// Test opening a file with create flag
			tieredFile, err = tieredFileSystemDriver.OpenFile("test4_LocalFileWithDifferentAccessFlags", os.O_CREATE, 0600)
			...
			// Test opening a file with create flag and read write flag
			tieredFile, err = tieredFileSystemDriver.OpenFile("test4_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_RDWR, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test5_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_APPEND, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test6_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_TRUNC, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test7_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_APPEND|os.O_RDWR, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test8_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_TRUNC|os.O_RDWR, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test9_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_APPEND|os.O_RDONLY, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test10_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_TRUNC|os.O_RDONLY, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test11_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0600)
			...
			tieredFile, err = tieredFileSystemDriver.OpenFile("test12_LocalFileWithDifferentAccessFlags", os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0600)
		...
		t.Run("KeepsCountOfOpenFiles", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("OnlyKeepsMaxFilesOpened", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
		...
		t.Run("FlushClearsDirtyLog", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			fsd := storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/object")
		...
		t.Run("SyncDirtyFiles_SkipsEmptyFiles", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
			...
			err := fs1.WriteFile("empty_file", []byte{}, 0600)
			if err != nil {
				t.Error(err)
			}

			// Create another file with content
			err = fs1.WriteFile("content_file", []byte("content"), 0600)
			if err != nil {
				t.Error(err)
			}
			...
			lowTierFSD := storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/object")
		...
		t.Run("FlushAndRestartScenario", func(t *testing.T) {
			fs1 := storage.NewFileSystem(
				storage.NewLocalFileSystemDriver(app.Config.StorageLocalPath + "/local"),
			)
...
func TestTieredFileSystemDriver_SyncDirtyFiles(t *testing.T) {
	...
	test.RunWithApp(t, func(app *server.App) {
		if os.Getenv("TEST_RUN") == "" {
			...
			cmd.Env = append(os.Environ(), "TEST_RUN=1", fmt.Sprintf("LITEBASE_TEST_DATA_PATH=%s", app.Config.StorageLocalPath), fmt.Sprintf("LITEBASE_TEST_ENCRYPTION_KEY=%s", app.Config.EncryptionKey))
			err := cmd.Run()
