...
/*
Permissions are defined by a list of resources and statements that can be
performed on those resources.

When an authentication credential has a rule with the resource defined as "*",
this indicates that the access rule is scoped to all resources. Rules can also
be scoped to a specific resource, such as a database, branch, or table.
...
When a credentials has a rule with Actions defined as "*", this indicates that
the credential rule is scoped to all actions. Actions can also be scoped to
...
*/
func (credential *Credential) authorizationKey(strs ...string) string {
	return strings.Join(strs, ":")
}

func (credential *Credential) authorizedForBranch(databaseId, branchId string, privilege Privilege) bool {
	// Any resource
	...
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "*"), privilege) {
		return true
	}

	// Any branch resource of the database
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", "*"), privilege) {
		return true
	}

	// Any resource of the specific branch of the database
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "*"), privilege) {
		return true
	}

	// A specific branch of a specific database
	return Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId), privilege)
}

func (credential *Credential) authorizedForColumn(databaseId, branchId, table, column string, privilege Privilege) bool {
	// Any resource
	if Authorized(credential.Statements(), credential.authorizationKey("*"), privilege) {
		return true
	}
	...
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "table", table, "*"), privilege) {
		return true
	}

	// Any column resource of the table
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "table", table, "column", "*"), privilege) {
		return true
	}

	// A specific column of a specific table of a specific database
	return Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "table", table, "column", column), privilege)
}
...
func (credential *Credential) authorizedForTable(databaseId, branchId, table string, privilege Privilege) bool {
	...
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "table", "*"), privilege) {
		return true
	}
	...
	return Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "table", table, "*"), privilege)
}

// Determine if an Access Key is authorized to perform an action on a module.
func (credential *Credential) authorizedForVTable(databaseId, branchId, module, vtable string, privilege Privilege) bool {
	// Any resource
	if Authorized(credential.Statements(), credential.authorizationKey("*"), privilege) {
		return true
	}
	...
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "module", "*"), privilege) {
		return true
	}

	// Any vtable resource of the specific module
	if Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "module", module, "vtable", "*"), privilege) {
		return true
	}

	// A specific vtable of a specific module of a specific database
	return Authorized(credential.Statements(), credential.authorizationKey("database", databaseId, "branch", branchId, "module", module, "vtable", vtable), privilege)
}
...
func (credential *Credential) CanAccessDatabase(databaseId, branchId string) error {
	...
	for _, statement := range credential.Statements() {
		if statement.Resource == "*" {
			return nil
		}

		if statement.Resource.HasPrefix(credential.authorizationKey(databaseId, branchId)) {
			return nil
		}
	}
...
func (credential *Credential) CanAnalyze(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeAnalyze) {
		return true, nil
	}
...
func (credential *Credential) CanAlterTable(databaseId, branchId, database, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeAlterTable) {
		return true, nil
	}
...
func (credential *Credential) CanCreateIndex(databaseId, branchId, tableName, indexName string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, tableName, DatabasePrivilegeCreateIndex) {
		return true, nil
	}
...
func (credential *Credential) CanCreateTable(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeCreateTable) {
		return true, nil
	}
...
func (credential *Credential) CanCreateTempTable(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeCreateTempTable) {
		return true, nil
	}
...
func (credential *Credential) CanCreateTempTrigger(databaseId, branchId, tableName, triggerName string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, tableName, DatabasePrivilegeCreateTempTrigger) {
		return true, nil
	}
...
func (credential *Credential) CanCreateTempView(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeCreateTempView) {
		return true, nil
	}
...
func (credential *Credential) CanCreateTrigger(databaseId, branchId, tableName, triggerName string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, tableName, DatabasePrivilegeCreateTrigger) {
		return true, nil
	}
...
func (credential *Credential) CanCreateView(databaseId, branchId, view string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, view, DatabasePrivilegeCreateView) {
		return true, nil
	}
...
func (credential *Credential) CanCreateVTable(databaseId, branchId, moduleName, vtable string) (bool, error) {
	if credential.authorizedForVTable(databaseId, branchId, moduleName, vtable, DatabasePrivilegeCreateVTable) {
		return true, nil
	}
...
func (credential *Credential) CanDelete(databaseId, branchId string, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeDelete) {
		return true, nil
	}
...
func (credential *Credential) CanDropIndex(databaseId, branchId, table, index string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeDropIndex) {
		return true, nil
	}
...
func (credential *Credential) CanDropTable(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeDropTable) {
		return true, nil
	}
...
func (credential *Credential) CanDropTrigger(databaseId, branchId, table, trigger string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeDropTrigger) {
		return true, nil
	}
...
func (credential *Credential) CanDropView(databaseId, branchId, view string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, view, DatabasePrivilegeDropView) {
		return true, nil
	}
...
func (credential *Credential) CanFunction(databaseId, branchId, function string) (bool, error) {
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegeFunction) {
		return true, nil
	}
...
func (credential *Credential) CanInsert(databaseId, branchId, table string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, table, DatabasePrivilegeInsert) {
		return true, nil
	}
...
func (credential *Credential) CanPragma(databaseId, branchId, pragma, value string) (bool, error) {
	...
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegePragma) {
		return true, nil
	}
...
func (credential *Credential) CanRead(databaseId, branchId, table, column string) (bool, error) {
	if credential.authorizedForColumn(databaseId, branchId, table, column, DatabasePrivilegeRead) {
		return true, nil
	}
...
func (credential *Credential) CanRecursive(databaseId, branchId string) (bool, error) {
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegeRecursive) {
		return true, nil
	}
...
func (credential *Credential) CanReindex(databaseId, branchId, index string) (bool, error) {
	if credential.authorizedForTable(databaseId, branchId, index, DatabasePrivilegeReindex) {
		return true, nil
	}
...
func (credential *Credential) CanSavepoint(databaseId, branchId, operation, savepoint string) (bool, error) {
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegeSavepoint) {
		return true, nil
	}
...
func (credential *Credential) CanSelect(databaseId, branchId string) (bool, error) {
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegeSelect) {
		return true, nil
	}
...
func (credential *Credential) CanTransaction(databaseId, branchId, operation string) (bool, error) {
	if credential.authorizedForBranch(databaseId, branchId, DatabasePrivilegeTransaction) {
		return true, nil
	}
...
func (credential *Credential) CanUpdate(databaseId, branchId, table, column string) (bool, error) {
	if credential.authorizedForColumn(databaseId, branchId, table, column, DatabasePrivilegeUpdate) {
		return true, nil
	}
