...
type TokenInput struct {
	...
	Statements  []TokenInputStatement `json:"statements"`
}

type TokenInputStatement struct {
...
func NewTokenCreateCmd(config *config.CLIConfiguration) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			input := TokenInput{
				Description: "",
				Statements: []TokenInputStatement{
					{
			...
			if nonInteractive {
				...
				form := components.NewForm(
					huh.NewGroup(
						huh.NewNote().
							Title("Create Token").
							Description("Add a description and define the statements for the token."),
						huh.NewInput().
							Title("Description").
					...
					huh.NewGroup(
						huh.NewText().
							Title("Statements").
							Description("Define privileges for this token using JSON").
							Lines(10).
							...
							Validate(func(str string) error {
								...
								// Ensure valid JSON
								var statements []TokenInputStatement
					...
					huh.NewGroup(
						huh.NewConfirm().
							Title("Confirm").
							Description("Are you sure you want to create this token?").
							Value(&confirmed),
					),
				)
			...
			res, _, err := api.Post(config, "/v1/tokens", map[string]any{
			...
			_, err = lipgloss.Fprint(
				...
				components.Container(
					...
					components.NewCard(
						...
						components.WithCardDescription("Copy and securely store the Token now. You won't be able to retrieve this later."),
