...
func NewTokenUpdateCmd(config *config.CLIConfiguration) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if nonInteractive {
				...
				form := components.NewForm(
					huh.NewGroup(
						huh.NewNote().
							Title("Update Token").
							Description("Update the description and the statements for the token."),
						huh.NewInput().
							Title("Description").
					...
					huh.NewGroup(
						huh.NewConfirm().
							Title("Confirm").
							Description("Are you sure you want to update this token?").
							Value(&confirmed),
					),
				)
			...
			if tokenId, ok := dataMap["tokenId"].(string); ok {
